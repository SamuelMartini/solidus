...
module Spree
  module Calculator::Shipping
    describe FlatPercentItemTotal, type: :model do
      let(:variant1) { build_stubbed(:variant, price: 10.11) }
      let(:variant2) { build_stubbed(:variant, price: 20.2222) }

      it_behaves_like 'a calculator with a description'

      let(:line_item1) { build_stubbed(:line_item, variant: variant1) }
      let(:line_item2) { build_stubbed(:line_item, variant: variant2) }
